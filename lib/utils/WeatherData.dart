...
class WeatherData {
  static var _weather = WeatherApi(windspeedUnit: WindspeedUnit.ms, temperatureUnit: TemperatureUnit.celsius);
  ...
  static void checkUnits() {
    ...
    if (_isCelsiusCurrently != isCelsius || _isKnotsCurrently != isKnots) {
        ...
        _weather = WeatherApi(temperatureUnit: isCelsius ? TemperatureUnit.celsius : TemperatureUnit.fahrenheit,
  ...
  static List<OpenMeteoLocation> locationsFromCoordList(
    ...
    return [
      for (final l in coords)
        OpenMeteoLocation(latitude: l.$1.toDouble(), longitude: l.$2.toDouble()),
    ];
  }
  ...
  static Future<Map<DateTime, List<Map<String, dynamic>>>> getMultipleData(
    ...
    for (int i = 0; i < response.segments.length; i++) {
      ...
      for (DateTime time in item.hourlyData[WeatherHourly.wind_speed_10m]?.values.keys ?? []) {
        if (!pointData.containsKey(time)) {
          pointData[time] = [];
        } 

        pointData[time]!.add({
          'latitude': locations[i].latitude, // use location from request not that returns since it returns nearest available locatoion with data, not to the exact location
          'longitude': locations[i].longitude,
          'windSpeed': item.hourlyData[WeatherHourly.wind_speed_10m]?.values[time] ?? 0.0,
          'gustSpeed': item.hourlyData[WeatherHourly.wind_gusts_10m]?.values[time] ?? 0.0,
          'windDirection': item.hourlyData[WeatherHourly.wind_direction_10m]?.values[time] ?? 0.0,
          'temperature': item.hourlyData[WeatherHourly.temperature_2m]?.values[time] ?? 0.0,
          'pressure': item.hourlyData[WeatherHourly.pressure_msl]?.values[time] ?? 0.0,
          'time': time,
          'index': 0,
        });
      }
  ...
  static Future<Map<DateTime, Map<String, dynamic>>> getSingleData(
    ...
    Map<DateTime, List<Map<String, dynamic>>> data = await getMultipleData(locations);
  ...
  static Future<List<Map<String, dynamic>>> getRouteData(
    ...
    final int sampleInterval = (routePoints.length / 20).ceil(); // Maximum 20 points
    ...
    return data.isEmpty ? [] : data[data.keys.last]!; 
    // ^ temporary so it matches previous format, if you want to access the points for different times just change this to return *data* and index it with a DateTime to get the same as before
  }
  ...
  static Future<Map<DateTime, Set<(num, num, num, num)>>> getWindData(
    ...
    for (DateTime time in data.keys) {
      ...
      for (Map<String, dynamic> item in data[time] ?? []) {
        dataOldFormat[time]!.add((item['latitude'] as num, item['longitude'] as num, item['windSpeed'] as num, item['windDirection'] as num));
      }
    }
